...
- TIL
- 숫자 + '' 을 하면 '숫자' 문자열로 바뀐다
- n의 제곱근은 약수보다 크며, for문 시 일일히 i를 올려주면서 n과 비교하지 않고도   
약수가 존재한다면? i를 제곱근까지 올려주면서 n과 i를 비교하면 된다  
예시) n = 45일 때 약수는 1,3,5,9,15,45가 존재함 제곱근은 6.7....  
3과 5는 제곱근 6보다 작은 걸 확인할 수 있다 그래서 n과 제곱근 전까지만 for문을 돌려준다면 나눠지는 수를 찾을 수 있다.

## 자료 구조를 왜 신경써야 하는 가? 
- 자료 구조들을 고유의 클래스 형태로 구현할 예정임.
- 공통적으로 가지고 있는 특징은 값들의 모임인 것. -> 자료 구조들은 해당 데이터에 적용되는 값들 및 기능 혹은 작업들 사이의 관계를 포함함. 
- 특정 유형의 문제에 있어서 특정한 자료 구조가 효율적이라는 사실에 기인
- 자료 구조들은 결국 데이터를 저장하는 동일한 기능을 제공하지만 동작 방식, 데이터와 기능들 사이의 관계, 사용되는 메소드들이 다 다름.  
...
## class 문법
```javascript
  class Student{ // 패턴을 정하는 것, 해당 class로 인스턴스화 될 객체의 패턴을 정하는 것.
    constructor(firstName,lastName){ //생성자 함수
      this.firstName = firstName;
      this.lastName = lastName;
    }
    fullName(){ 
      //내가 만든 배열(Array에 인스턴스)이 array메소드를 사용할 수 있는 것 처럼 Student로 만들어진 객체는 Student의 메소드를 사용할 수 있음 해당this는 인스턴스를 가르킴
      return `your Name is ${firstName} ${lastName}`
    }
    static EnrollStudents(){ // 인스턴스에선 사용할 수 없음 Student 클래스 메서드임
      return "HI"
    }
  }
  let firstStudent = new Student('lee','mk')
  //firstStudent ={firstName = 'lee', lastName = 'mk'}
  // new => 클래스로부터 새로운 객체를 인스턴스화 하는 방식
  // this는 new로 생성되는 인스턴스 객체를 의미함. thissms 


  // statice 메서드 부연 설명
  class Point{
    constructor(x,y){//생성자 함수. new 사용시 클래스의 인자로 넣으면 해당 객체에 아래 구문이 포함됨
      this.x = x;
      this.y = y;
    }
    static distance(a,b){
      ...
      return Math.hypot(dx,dy); //두 좌표 사이에 거리를 측정 
    }
  }
  ...
  // 위처럼 코드를 작성하는 이유는 보기 좋도록, 이해하기 쉽도록 코드를 구성하고 논리적으로 표현하기 위함. 
```

## 자료구조 알고리즘
1. Singly Linked Lists (단일 구조 알고리즘)
  - 클래스를 만들어 놓고 새로운 노드들을 선형구조로 연결하는 것.
2. 스택과 큐()
  - 스택
     - 압축한 데이터 모음 - 후입선출 원칙을 따름 - 나중에 들어온 것이 먼저 나옴 -> 마지막으로 추가된 요소가  
     먼저 지워짐
     - 재귀함수에 스택이 쌓였을 때 최종부터 지워지는 것을 참고 할 수 있음
     - 뒤로 가기 / 앞으로 가기
     - 방문 기록 
  - 큐 
     - 압축한 데이터 모음 - 선입선출 원칙 
     - push로 추가하고, 제거할 땐 앞에서 부터 제거
     - 다운로드 할때 먼저 입력된 것들이 처리하게 하는 것. 프린터 출력 시 먼저 출력 걸어놓은 게 출력되는 것
3. 이진 검색 트리(Binary Research Tree) : 이진 트리는 노드의 자식이 최대 2개인 것
  - 트리 : 연결 리스트처럼 노드로 이루어진 데이터 구조
  - 노드들 사이에 부모 자식 구조가 형성됨.
  - 탑 다운 방식이 많음. 노드는 자식구조만 가르킬 수 있음, 출발 지점은 하나여야만 함
  - 용어
    - 루트 : 트리 꼭대기에 있는 노드
    - child : 노드의 자식
    - parent : 노드의 부모
  ...
  - 이진트리 특정
    - 자식 노드는 최대 2개
    - 부모노드에 오른쪽은 부모노드보다 큰숫자, 왼쪽은 부모노드보다 작은 숫자
    - 위 규칙으로 인해 숫자가 들어옴에 따라 탐색해야 하는 범위가 반이상으로 줄어든다.
4. 트리 순회
  - 트리 구조에서 모두 사용할 수 있음.(어떻게 트리를 전체 순회하는 가?)
  - 선형 구조에서는 편하게 할 수 있지만 비선형 구조에서는 재귀를 사용하는 것이 좋음
  - 너비 우선 방식(Breadth-first Search)
    - 처음부터 시작해서 기본적으로 트리를 가로질러서 작업
    - 좌에서 우로 탐색. root 좌-우 노드 
    - 큐를 만들어야 함(선입선출 구조)
  - 깊이 우선 방식(Depth-first Search)
    - 제일 아래 노드로 내려가서 올라올 것인지에 대한 정의?
    - 트리 구조라면 기준을 정해놓고 해당 기준의 모든 것을 순회한 뒤 다시 root로 이동
